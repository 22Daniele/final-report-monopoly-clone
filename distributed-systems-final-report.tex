...
It comprehends the serialized state of the game (players, turn, board \ldots) and the players token.
...
They are then used after a reconnection from a player by checking if the token provided by the user is the same of the token saved
on the server.
...
If these files gets corrupted or lost, game cannot be restored without checkpoint or disconnected players cannot rejoin without token.
There is a single copy of them, there isn't a backup, and they are not shared between nodes.
This increase the consistency at the cost of availability and fault tolerance.
