...
The game continues until only one player remains without going in \textit{bankruptcy}
or until all the players leave the game.
...
The system's design is a mix of a \textbf{3-Tier Architecture} with an \textbf{Event-Based Architecture}.


The system is logically and physically divided into three distinct tiers:
\begin{itemize}
    \item \textbf{Client (Presentation Tier):} Responsible exclusively for the user interface (\texttt{PyGame}).
    \item \textbf{Server (Application/Logic Tier):} It contains the game logic and acts as the authoritative coordinator.
    \item \textbf{Filesystem (Data Tier):} The components interact directly with the local file system to store information.
\end{itemize}


The communication between the components is inherently \textbf{event-driven}.
The Server does not run a continuous computation loop, instead it waits passively for network events or system events.
Once an event is received, it triggers a state mutation on the Application Tier,
which generates a reactive broadcast event (the updated JSON state) sent back to the Presentation Tier.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/model_diagram}
...
They only send user events and wait.
The Server is the sole authority that executes the logic and determines the outcome.
Instead of clients polling the server for updates, the server proactively broadcasts the entire
serialized game state to all connected peers every time a mutation occurs.
Upon receiving a new state, the client's GUI inspects the \texttt{current\_turn\_nickname} and \texttt{allowed\_actions} fields
to dynamically determine whether to enable the input buttons for the local user.


\subsubsection*{Communication Flow}
Communication is triggered by user inputs (e.g., clicking a button) or connection lifecycle events (e.g., socket closed).
Clients send lightweight JSON payloads containing an action type (e.g., \texttt{join}, \texttt{ready}, \texttt{roll})
and a payload specifying the player's nickname and token.
The server responds by broadcasting the full JSON representation of the entire Monopoly object,
ensuring all clients are perfectly synchronized.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/sequence_diagram}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/state_diagram}
...
\subsubsection{Network Partitions and Heartbeat Mechanism}
On network drops, the system implements an application-level \textbf{Heartbeat} mechanism coupled
with a 5-second socket timeout on the \texttt{recv()} function.
Active clients send a lightweight \texttt{"ping"} message every 2 seconds in the background.
The server intercepts these messages and immediately replies with a \texttt{"pong"}, without interfering with the game logic.
If a silent drop occurs, the 5-second timeout triggers a socket exception, immediately starting the standard disconnection protocol.
...
The standard game loop (joining, rolling, buying, ending turns) was tested focusing also on corner cases and fault-tolerance mechanisms:
  \begin{itemize}
      ...
      \item \textit{Network Partitions:} Unexpected client and server disconnections were simulated.
                    The system successfully detected the network partition within 5 seconds thanks to the Heartbeat mechanism,
                    triggering the dynamic 30-second or 90-second timeouts and the subsequent recovery or bankruptcy logic.
...
\begin{enumerate}
    ...
    cd DS-monopoly-clone/dmonopoly
    \end{verbatim}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/lobby}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/playing}
    \caption{The game interface during the PLAYING phase.}
    \label{fig:gui_playing}
\end{figure}

\subsection{PAUSED phase}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/paused}
    \caption{The game interface during the PAUSED phase.}
    \label{fig:gui_paused}
\end{figure}

\subsection{CLOSED phase}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/closed}
...
The system was completely designed and implemented individually by Daniele Loverre.

\subsubsection*{Strengths}
\begin{itemize}
    \item \textbf{Fault Tolerance:} The system handles sudden crashes and network drops effectively.
    The checkpointing and timeout mechanisms ensure that the game can be easily recovered without any data loss.
    \item \textbf{Decoupled Architecture:} The architecture was structured to be lightweight and reusable,
    strictly decoupling the network layer from the Model and the View.
    \item \textbf{Anti-Cheating Design:} Having the clients act solely as passive renderers of the View keeps the system safe from cheating,
    allowing players to only perform legally allowed actions.
    \item \textbf{Network Reliability and Efficiency:} The use of TCP sockets makes the connections
    more reliable and makes it easier to detect connection drops.
    Furthermore, the client-server architecture keeps the network lightweight for each client,
    which only needs a single connection with the server (avoiding the network overload of a peer-to-peer mesh).
    The messages exchanged are small and fast, allowing the server to instantly broadcast the game state to all connected clients.
\end{itemize}

\subsubsection*{Weaknesses of the product}
\begin{itemize}
    \item \textbf{Single Point of Failure:} Having the server as the single coordinator of the game introduces a Single Point of Failure.
    Even though a recovery mechanism is implemented, if the server fails and cannot be restarted to reestablish connections
    within the 90-second client timeout, the current game session is lost and there is no way to restore the game state.
    \item \textbf{Manual Network Discovery:} When connecting across different physical machines,
    especially if the server is hosted via Docker, users must manually look up and input the host's IP address,
    which makes finding the server less user-friendly.
    \item \textbf{Limited Security:} Security is limited as a trade-off for simplicity and performance.
    Authentication tokens are stored in clear text, and TCP payloads are unencrypted.
    \item \textbf{Static GUI:} The graphical user interface is extremely simple and lacks dynamic responsiveness to different monitor resolutions or window resizing.
    \item \textbf{Automated Test Coverage:} Another weakness is the limited scope of automated testing.
    Only the core domain logic and serializers are covered by automated unit tests.
    The integration and network behaviors were tested manually, potentially leaving some rare game configurations or edge cases untested.
\end{itemize}

\section{Future works}\label{future-works}

Possible future work and improvements could be:
\begin{itemize}
    \item \textbf{Web-Based Client Interface:} Replacing the current \texttt{PyGame} desktop GUI with a web-based frontend.
    This would improve the user experience by providing responsive design across different devices
    and removing the need for local installations, simplifying the deployment process.
    \item \textbf{Removing the Single Point of Failure (SPOF):} As discussed in Section~\ref{self-evaluation},
    the current architecture relies on a single authoritative server.
    A more robust and highly available setup would involve deploying multiple server nodes behind a Load Balancer,
    and a distributed ledger so that if one server node crashes, another can seamlessly pick up the ongoing session.
    \item \textbf{Concurrent Game Sessions (Matchmaking):} The current system hosts a single game at a time.
    An extension would be upgrading the server into a central hub where users can create and join multiple concurrent
    public or private game rooms (lobbies), each running as an isolated game thread.
    \item \textbf{Account Management System:} Integrating a database to handle proper user accounts.
    This would allow players to register with a username and password, keeping track of their game history, win rates, and friends lists.
    Consequently, a more robust authentication and authorization mechanism would replace the current one.
    \item \textbf{Strengthened Security:} Introducing a layer of encryption on exchanged data and saved files on both server and client sides.
    \item \textbf{Adding game feature:} The system implements only few feature of the real board game, a lot of feature can be added to make the game complete.
\end{itemize}
\end{document}
